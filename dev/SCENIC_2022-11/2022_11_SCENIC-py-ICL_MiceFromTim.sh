...
if [[ 0 -eq 1 ]]; then
  ...
  sbatch --output=slurm-scenic-${loomfilename}-%x.%j.out --job-name=SCN_TIM -c ${processors} --time=1-00:00:00 --mem=${mem} --export=ALL,loomfilename="${loomfilename}" ${script_dir}/2022_Tim/2022_11_SCENIC-py-ICL_MiceFromTim.sh
...
f_db_names="${scenicdatadir}mm9-tss-centered-10kb-7species.mc9nr.genes_vs_motifs.rankings  ${scenicdatadir}mm9-500bp-upstream-7species.mc9nr.genes_vs_motifs.rankings.feather"
